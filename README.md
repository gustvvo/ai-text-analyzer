...
**Auditability.** Any stored analysis — success or failure — is fully reconstructible: which prompt version built the request, which model and provider answered it, how many tokens it cost, and exactly when it happened. Three columns complete a minimal per-row replayable trace: `raw_response` (the provider's exact text before normalization/validation — the malformed text itself on an invalid-JSON failure, `null` only when no attempt ever returned output, e.g. a ProviderError on the first call; if any attempt returned text, the last-seen raw text is preserved even when a later attempt fails), `duration_ms` (wall time of the full `analyze()` pipeline, including any retry), and `attempts` (1 on the happy path, 2 when the shape-failure retry ran). Together they answer "what did the model actually say, how long did it take, and did we retry?" per row, straight from `psql`, with no log correlation needed.
