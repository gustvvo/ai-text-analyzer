...
Backend: `npm test` (from `backend/`), which runs `vitest run`. As of this writing, that's **115 tests across 16 files**, all passing, with no external services required (the analysis pipeline is tested against the real `MockProvider` and a mocked repository, not a live database).
